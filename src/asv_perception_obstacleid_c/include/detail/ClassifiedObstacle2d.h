...
//             const float left = std::max(x0, x1); 
//             const float right = std::min(x0 + w0, x1 + w1); 
//             const float bottom = std::min(y0 + h0, y1 + h1); 
//             const float top = std::max(y0, y1);

//             return ((left < right) && (bottom > top))? 
//                     (bottom - top) * (right - left) : 0.f;
//         }   // rect_intersection
...
//         //     const float depth, const float height, const float z_offset, 
//         //     const Homography& h) 
//         // {
...
//             ClassifiedObstacle2d(Classification cls) : cls(std::move(cls)), parent() {}
...
//             //     const float min_depth, float max_depth) const 
//             // {    
...
namespace impl {
    ...
    using namespace asv_perception_interfaces;
...
class ClassifiedObstacle2d {
    ...
    public:
        ...
        asv_perception_interfaces::msg::Classification cls;
        std::weak_ptr<ClassifiedObstacle2d> parent;

        ClassifiedObstacle2d() = default;
        ClassifiedObstacle2d( asv_perception_interfaces::msg::Classification cls )
            : cls(std::move(cls))
            , parent()
        {}
        ...
        // projects to 3D using homography and creates Obstacle message 
        asv_perception_interfaces::msg::Obstacle project( 
            const Homography& h
