...
#include <geometry_msgs/msg/point32.hpp>  // geometry_msgs/Polygon.points is Point32
...
namespace impl {
    ...
    inline sensor_msgs::msg::RegionOfInterest adjust_vertical_roi_to_image( 
...
)
{
    ...
    auto result = std::vector<msg::Obstacle>{};
    const auto 
    ...
    for ( auto& obs2d : obstacles_2d ) {
        ...
        if ( impl::is_above_horizon( *obs2d, h ) ) {
            RCLCPP_INFO_STREAM(rclcpp::get_logger("rclcpp"), std::string( "Image ROI above horizon, ignoring" ) );
            continue;
        }
