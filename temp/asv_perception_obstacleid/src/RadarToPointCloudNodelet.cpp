...
namespace {
  ...
  static const std::string 
    TOPIC_NAME_INPUT = "input", TOPIC_NAME_SEGMENT = "segment", 
    TOPIC_NAME_FULL = "full", TOPIC_NAME_FULL_CURRENT = "full_current";
...
void RadarToPointCloudNodelet::onInit ()
{
  ...
  int val = 0;  
  pnh_->declare_parameter("min_intensity", val);

  // get parameters
  if ( pnh_->get_parameter("min_intensity").as_int() && ( val >= 0 ) )
...
void RadarToPointCloudNodelet::subscribe ()
{
  ...
  this->sub_ = pnh_->subscribe<asv_perception_common::RadarSegment> (TOPIC_NAME_INPUT, 100, 
...
void RadarToPointCloudNodelet::sub_callback (const asv_perception_common::RadarSegment::ConstPtr& segment)
{
